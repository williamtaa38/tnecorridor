document.addEventListener("DOMContentLoaded", async function(){
  ...
  function renderStudents(){
    ...
    $("studentTable").innerHTML=rows.map(u=>{const count=applications.filter(a=>a.user_id===u.id || String(a.email||'').toLowerCase()===String(u.email||'').toLowerCase()).length; return `<tr><td><strong>${esc(u.name||'Student')}</strong><br><small>${esc(u.email)}</small></td><td>—</td><td><span class="status ${u.status==='active'?'accepted':'inactive'}">${esc(u.status)}</span></td><td>${count}</td><td><button class="btn-small" data-reset-email="${esc(u.email)}">Reset Password</button> <button class="btn-small" data-user-status="${esc(u.id)}" data-next-status="${u.status==='active'?'inactive':'active'}">${u.status==='active'?'Deactivate':'Activate'}</button></td></tr>`}).join('') || '<tr><td colspan="5">No student accounts found.</td></tr>';
  }
  ...
  $("studentForm")?.addEventListener('submit',async e=>{e.preventDefault(); const temporaryPassword=$("newStudentPassword").value; try{await api({action:'create',email:$("newStudentEmail").value.trim().toLowerCase(),name:$("newStudentName").value.trim(),role:'student',temporaryPassword}); $("studentModal").hidden=true; await load(); toast('Student account created. Ask the student to use Reset Password before first sign-in.');}catch(err){toast(err.message)}});
  ...
  $("adminLogoutBtn")?.addEventListener('click',async()=>{await supabase.auth.signOut(); sessionStorage.removeItem('tneStaffPreviewSession'); location.href='/pages/staff-login.html';});
