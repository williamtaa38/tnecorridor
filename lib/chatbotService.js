...
function hasConfirmedAcademicResults(state) {
  return !!(
    state?.spm?.received &&
    state?.academicResult?.confirmed &&
    Array.isArray(state?.academicResult?.subjects) &&
    state.academicResult.subjects.length
  );
}

function evaluationActionButtons(state) {
  const buttons = [];

  if (!hasConfirmedAcademicResults(state)) {
    buttons.push({ label: "Upload result now", action: "upload_result" });
  }

  return [...buttons, ...finalActionButtons()];
}
...
function reply(replyText, extras = {}) {
  return {
    ...
    downloadPdfLabel: extras.downloadPdfLabel || "Download eligibility PDF",
...
function stageReminder(state) {
  ...
  if (stage === "done") {
    return hasConfirmedAcademicResults(state)
      ? "Your completed EduSeek evaluation is ready. You can download the report, prepare the remaining application documents, or start a new evaluation."
      : "Your evaluation is still incomplete because no confirmed academic result is on file. Upload your result now to unlock a completed report and EduSeek Admission Readiness Score.";
  }
...
Admission Readiness Score: 0-100 or Not Available
Readiness Band: Strong / Competitive / Borderline / Needs Review
Reason:
Next step:

Rules:
- The Admission Readiness Score is an advisory counselling score, not an official acceptance probability.
- Only provide a numeric score when confirmed academic results are available. If results are missing or empty, use Not Available and Needs Review.
- 85-100 = Strong, 70-84 = Competitive, 50-69 = Borderline, below 50 = Needs Review.
...
async function buildAdmissionEstimate(bundle, state, eligibilitySummary) {
  if (!hasConfirmedAcademicResults(state)) return null;

  const text = String(eligibilitySummary || "");
  const scoreMatch = text.match(/Admission Readiness Score:\s*(\d{1,3})/i);
  const bandMatch = text.match(/Readiness Band:\s*(Strong|Competitive|Borderline|Needs Review)/i);
  const reasonMatch = text.match(/Reason:\s*([\s\S]*?)(?=\n(?:Next step:|$))/i);
  const nextStepMatch = text.match(/Next step:\s*([\s\S]*)$/i);
  const statusMatch = text.match(/Status:\s*(Eligible|Possibly Eligible|Need Review)/i);

  let score = scoreMatch ? Number(scoreMatch[1]) : null;
  let band = bandMatch ? bandMatch[1] : "";

  if (!Number.isFinite(score)) {
    const status = String(statusMatch?.[1] || "Need Review").toLowerCase();
    if (status === "eligible") score = 88;
    else if (status === "possibly eligible") score = 72;
    else score = 55;
  }
  ...
  const rationale = String(reasonMatch?.[1] || "The score reflects the confirmed results compared with the entry requirements currently available in EduSeek.").trim();
  const nextStep = String(nextStepMatch?.[1] || "").trim();

  return {
    score,
    band,
    rationale,
    missingItems: nextStep ? [nextStep] : []
  };
}

function evaluationCompletionText(state) {
  if (!hasConfirmedAcademicResults(state)) {
    return (
      "\n\n⚠️ Evaluation incomplete — no confirmed academic result has been uploaded. " +
      "Upload your result now so EduSeek can compare your grades with the selected programme requirements and produce a more complete counselling report. " +
      "A completed evaluation can include an EduSeek Admission Readiness Score and an indicative admission-likelihood assessment to help you prepare for an application. " +
      "This remains advisory and is not an official university admission decision."
    );
  }

  const estimate = state?.admissionEstimate;
  if (!estimate) {
    return (
      "\n\n✅ Your academic result is confirmed. Your evaluation is complete enough for a full EduSeek counselling report. " +
      "Final admission remains subject to the university's own assessment."
    );
  }

  const missing = estimate.missingItems?.length
    ? `\nItems still to confirm: ${estimate.missingItems.join("; ")}`
    : "";

  return (
    `\n\n✅ Evaluation complete.\n` +
    `EduSeek Admission Readiness Score: ${estimate.score}/100 (${estimate.band})\n` +
    `${estimate.rationale}${missing}\n\n` +
    "You can now download the completed EduSeek counselling report. The score is an advisory estimate based on the information available to EduSeek; it is not a guarantee or an official university decision."
  );
}
  ...
  try {
    ...
    if (state.stage === "await_spm") {
      ...
      if (["skip", "no", "later", "skip result", "skip spm"].includes(t)) {
        ...
        const r = reply(
          "You can continue without uploading a result, but your academic evaluation will remain incomplete.\n\nFor a useful admissions-oriented evaluation, upload your latest result when you are ready. Once the grades are confirmed, EduSeek can compare them with the selected programme requirements and produce a completed counselling report with an Admission Readiness Score and indicative admission-likelihood assessment. This is advisory and not an official university decision.\n\nWhat would you like to do next?",
          {
            actionButtons: [{ label: "Upload result now", action: "upload_result" }],
            quickReplies: [
    ...
    if (state.stage === "pick_course") {
      ...
      if (n && n >= 1 && n <= options.length) {
        ...
        state.admissionEstimate = await buildAdmissionEstimate(bundle, state, eligibility);
        state.stage = "done";
        session.state = state;
        ...
        replyText += evaluationCompletionText(state);
        replyText += "\n\nWhat would you like to do next?";

        const complete = hasConfirmedAcademicResults(state);
        const r = reply(replyText, {
          showDownloadPdf: true,
          downloadPdfLabel: complete
            ? "Download completed EduSeek report"
            : "Download preliminary report",
          actionButtons: evaluationActionButtons(state)
        });
      ...
      if (bundleByName?.course?.id) {
        ...
        state.eligibilitySummary = eligibility;
        state.admissionEstimate = await buildAdmissionEstimate(bundleByName, state, eligibility);
        state.stage = "done";
        session.state = state;
    ...
    if (state.stage === "done") {
      ...
      const r = reply(
        complete
          ? "Your EduSeek evaluation is complete. You can download your completed counselling report, review your next step, or start a new evaluation."
          : "Your preliminary report is ready, but the evaluation is incomplete because no confirmed academic result is on file. Upload your result now to unlock a completed evaluation and EduSeek Admission Readiness Score.",
        {
          showDownloadPdf: true,
          downloadPdfLabel: complete
            ? "Download completed EduSeek report"
    ...
    if (state.selectedCourseId) {
      ...
      r = reply(
        `✅ Results confirmed.\n\nDetected subjects (${cleaned.subjects.length}):\n${preview}\n\n` +
          `Your previously selected course has now been re-evaluated with the confirmed results.\n\n` +
          `Preliminary eligibility:\n${eligibility}` +
          evaluationCompletionText(state) +
          "\n\nNext step: download the completed report and use it as a counselling summary while preparing your application.",
        {
          showDownloadPdf: true,
          downloadPdfLabel: "Download completed EduSeek report",
          actionButtons: evaluationActionButtons(state)
        }
      );
    } else {
      ...
      r = reply(
        `✅ Results confirmed.\n\nDetected subjects (${cleaned.subjects.length}):\n${preview}\n\nYour academic data is now complete enough for a stronger course evaluation. What would you like to do next?`,
        {
          quickReplies: [
            quick("1. Do Personality test for more accurate result for career", "Option_Personality"),
            quick("2. Directly provide the Courses they are suitable", "Option_Direct_Courses"),
...
export async function generateEligibilityPdf(sessionId) {
  try {
    ...
    if (complete && !state.admissionEstimate) {
      state.admissionEstimate = await buildAdmissionEstimate(bundle, state, eligibility);
      session.state = state;
      await saveSession(session);
    }

    const estimate = state.admissionEstimate || null;
    ...
    const reportId = `EDU-${Date.now()}-${String(sessionId).replace(/[^a-zA-Z0-9]/g, "").slice(-6).toUpperCase()}`;
    ...
    function addPage() {
      ...
      page.drawText("EduSeek AI • Student Counselling Report", {
        x: LEFT,
        y: 815,
        size: 8,
        font,
    ...
    function drawText(text, opts = {}) {
      ...
      const lineGap = opts.lineGap || Math.max(14, size + 4);
      const fontRef = opts.bold ? fontBold : font;
      const color = opts.color || rgb(0.08, 0.12, 0.18);
      const maxChars = opts.maxChars || Math.max(48, Math.floor(CONTENT_W / (size * 0.53)));
      ...
      for (const line of wrapped) {
        ...
        if (line) {
          page.drawText(line, {
            x: opts.indent ? LEFT + opts.indent : LEFT,
            y,
            size,
            font: fontRef,
            color
    ...
    function section(title) {
      ensureSpace(34);
      y -= 5;
      drawText(title, { bold: true, size: 13.5, lineGap: 19, color: rgb(0.04, 0.12, 0.25) });
      drawRule();
    }

    function keyValue(label, value) {
      drawText(`${label}: ${value || "-"}`, { size: 10.5, lineGap: 15 });
    }

    addPage();

    drawText("EduSeek AI Education Evaluation Report", {
      bold: true,
      size: 20,
      lineGap: 27,
      color: rgb(0.03, 0.12, 0.27)
    });
    drawText(
      complete ? "COMPLETED EVALUATION" : "PRELIMINARY EVALUATION — ACADEMIC RESULT REQUIRED",
      {
        bold: true,
        size: 11,
        lineGap: 20,
        color: complete ? rgb(0.03, 0.42, 0.24) : rgb(0.68, 0.34, 0.02)
      }
    );
    drawText(`Generated: ${generatedAt.toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}`, {
      size: 9.5,
      color: rgb(0.35, 0.4, 0.48)
    });
    ...
    if (!complete) {
      ensureSpace(90);
      drawText(
        "Evaluation incomplete: no confirmed academic result has been uploaded. Upload your result in EduSeek AI to compare your grades with the selected programme requirements and unlock a completed counselling report with an EduSeek Admission Readiness Score.",
        { bold: true, size: 11, lineGap: 16, color: rgb(0.55, 0.26, 0.02) }
      );
    }

    section("1. Student Profile");
    keyValue("Student Name", student.fullName);
    keyValue("Email", student.email);
    keyValue("Phone", student.phone);
    keyValue("Preferred Course", student.intendedCourse);
    keyValue("Preferred University", student.intendedUniversity);

    section("2. Selected Programme");
    keyValue("Course", course.title);
    keyValue("Course Code", course.courseCode);
    keyValue("University", university);
    keyValue("Level", course.level);
    keyValue("Duration", course.duration);
    keyValue("Study Mode", course.studyMode);
    keyValue("Intake", course.intake);
    keyValue("Malaysian Tuition", formatMoney(course.tuitionCurrency, course.tuitionTotal_Malaysian));
    keyValue("International Tuition", formatMoney(course.tuitionCurrency, course.tuitionTotal_International));

    section("3. Academic Result Used for Evaluation");
    if (subjects.length) {
      subjects.forEach((subject) => {
        drawText(`• ${subject.subject}: ${subject.grade}`, { size: 10.5, lineGap: 15 });
      });
    } else {
      drawText("No confirmed academic results uploaded.", { bold: true, color: rgb(0.65, 0.28, 0.02) });
      drawText("Next action: return to EduSeek AI and choose ‘Upload result now’ to complete the evaluation.");
    }

    section("4. Preliminary Eligibility Assessment");
    drawText(eligibility, { size: 10.5, lineGap: 15 });

    if (complete) {
      section("5. EduSeek Admission Readiness Score");
      if (estimate) {
        drawText(`${estimate.score}/100 — ${estimate.band}`, {
          bold: true,
          size: 18,
          lineGap: 25,
          color: rgb(0.03, 0.35, 0.22)
        });
        drawText(estimate.rationale || "Further admissions review is recommended.");
        if (estimate.missingItems?.length) {
          drawText("Items still to confirm:", { bold: true });
          estimate.missingItems.forEach((item) => drawText(`• ${item}`));
        }
      } else {
        drawText("Admission Readiness Score could not be produced from the available information.");
      }
      drawText(
        "Important: this score is an EduSeek advisory estimate based on confirmed results and the database requirements visible at the time of evaluation. It is not a guaranteed acceptance probability, an offer, or an official university admission decision.",
        { size: 9.5, color: rgb(0.35, 0.4, 0.48) }
      );
    } else {
      section("5. What to Do Next");
      drawText("1. Upload your latest academic result in EduSeek AI.", { bold: true });
      drawText("2. Review the detected subjects and grades, then confirm them.");
      drawText("3. EduSeek will re-evaluate this selected course against the available entry requirements.");
      drawText("4. Download the completed report with an Admission Readiness Score and indicative admission-likelihood assessment.");
    }

    section(complete ? "6. Database Entry Requirements" : "6. Database Entry Requirements");
    if (requirements.length) {
      for (const req of requirements) {
        drawText(`Qualification: ${req.qualification || "-"}`, { bold: true });
        drawText(`Minimum Requirement: ${req.minimumRequirement || "-"}`);
        drawText(`English Requirement: ${req.englishRequirement || "-"}`);
        if (req.notes) drawText(`Notes: ${req.notes}`);
        y -= 5;
      }
    } else {
      drawText("No entry requirements were found in the current EduSeek database for this programme. Confirm requirements directly with the university before applying.");
    }

    section("7. Relevant Scholarships / Financial Aid");
    if (scholarships.length) {
      for (const scholarship of scholarships.slice(0, 8)) {
        drawText(`${scholarship.title || "Scholarship"} (${scholarship.scholarshipType || "-"})`, { bold: true });
        drawText(`Benefit: ${scholarship.amountOrBenefit || "-"}`);
        drawText(`Eligibility: ${scholarship.eligibilityCriteria || "-"}`);
        drawText(`Intake: ${scholarship.applicableIntake || "-"}`);
        drawText(`Deadline: ${scholarship.deadline || "-"}`);
        y -= 5;
      }
    } else {
      drawText("No matching scholarship was found in the current EduSeek database. An admissions officer may still be able to check other funding options.");
    }

    section("8. Recommended Next Steps");
    if (complete) {
      drawText("1. Check any outstanding requirements shown in this report.");
      drawText("2. Prepare official certificates/transcripts and English-language evidence where required.");
      drawText("3. Review scholarship deadlines and supporting documents.");
      drawText("4. Proceed to the TNE Corridor application process when your documents are ready.");
      drawText("5. Treat EduSeek's score as counselling guidance; the university makes the final admission decision.");
    } else {
      drawText("Your most useful next step is to upload and confirm your academic result in EduSeek AI. Without verified grades, the evaluation cannot be completed and no meaningful readiness score should be issued.", { bold: true });
    }

    if (complete) {
      section("EduSeek AI Evaluation Completion Certificate");
      drawText(`This certifies that ${student.fullName || "the student"} completed an EduSeek AI counselling evaluation for:`, {
        size: 11
      });
      drawText(`${course.title || "Selected programme"} — ${university}`, { bold: true, size: 12 });
      if (estimate) {
        drawText(`EduSeek Admission Readiness Score: ${estimate.score}/100 (${estimate.band})`, {
          bold: true,
          size: 13,
          color: rgb(0.03, 0.35, 0.22)
        });
      ...
      drawText(`Issued: ${generatedAt.toLocaleDateString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}`);
      drawText(
        "Advisory certificate only. This document is generated by EduSeek AI for education counselling and application preparation. It does not replace official academic documents, does not guarantee admission, and is not an offer letter from the university.",
        { size: 9.5, color: rgb(0.35, 0.4, 0.48) }
      );
    }
    ...
    pages.forEach((p, index) => {
      ...
      p.drawText("EduSeek AI — advisory education counselling document", {
        x: LEFT,
        y: 30,
        size: 8,
        font,
    ...
    const fileName = complete
      ? `eduseek-completed-evaluation-${Date.now()}.pdf`
      : `eduseek-preliminary-evaluation-${Date.now()}.pdf`;
    ...
    const { error: reportError } = await supabaseAdmin.from("reports").insert({
      ...
      report_json: {
        ...
        scholarships: scholarships.slice(0, 8).map((scholarship) => ({
          title: scholarship.title || "",
          scholarshipCode: scholarship.scholarshipCode || "",
          scholarshipType: scholarship.scholarshipType || "",
          amountOrBenefit: scholarship.amountOrBenefit || ""
        })),
